...
class AlmacenTransportesController extends AppController {
 ...
 public function form ($id = null) { //esta accion vale tanto para edit como add
	...
	/*	$linea_almacen = $this->AlmacenTransporte->findById($id);
		$transporte_id = $linea_almacen['Transporte']['id'];
	}else{ //un add() obtenemos la id
		$transporte_id = $this->params['named']['from_id'];
	}

	$almacen_transporte = $this->AlmacenTransporte->Transporte->find(
		'first',
		array(
			'conditions' => array(
