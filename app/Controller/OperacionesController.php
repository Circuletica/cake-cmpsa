...
//	$this->set(compact('invoice');
	...
	$transportes = $this->Operacion->Transportes->find(
	    'all',
	    array(
		'recursive' => 2,
		'contain' => array(
...
/*
	...
	//$transporte = $this->Operacion->Transporte->find()
		...
		foreach ($operaciones as $operacion){
				if (isset($operacion['Contrato']['si_entrega'])) {
				  $entrega = $operacion['Contrato']['si_entrega'] ? 'Entrega' : 'Embarque';
					...
					$mes = substr($fecha,5,2);
					$anyo = substr($fecha,0,4);
					$fecha_transporte = $dia.'-'.$mes.'-'.$anyo;
				  	$entrega = $fecha_transporte.' ('.$entrega.')';
				}else{ 
				 	$entrega = $fecha_transporte.'';
				}	
			...
			$situacion_embarques[] = array(
					'Calidad'=> $operacion['Contrato']['CalidadNombre']['nombre'],
					'Operacion' => $operacion['Operacion']['referencia'],
					'Proveedor' => $operacion['Contrato']['Proveedor']['nombre_corto'],
					'T.M.' => $operacion['Contrato']['Proveedor']['nombre_corto'],
					'Embarque' => $entrega,	
					'Pto.Destino' => $operacion['PuertoDestino']['nombre'],	
					'Fecha carga' =>$fecha_carga,// $operacion['Operacion']['Transporte']['fecha_carga'],	
					/*'Fecha llegada' => $operacion['Operacion']['Transporte']['fecha_llegada'],
					'Vehiculo' => $operacion['Operacion']['Transporte']['nombre_vehiculo'],*/
					'Llegada prevista' => $operacion['Operacion']['id']
					);
			}
